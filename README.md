...
- **Psalm**
    ...
    ```bash
    docker-compose run --rm tests vendor/bin/psalm
    ```
...
- **PHPCS**
    ...
    ```bash
    docker-compose run --rm tests vendor/bin/phpcs -s --report-source --runtime-set ignore_warnings_on_exit 1
    docker-compose run --rm tests vendor/bin/phpcbf
    ```
